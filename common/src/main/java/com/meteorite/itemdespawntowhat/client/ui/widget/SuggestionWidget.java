...
/*
 * ID输入建议组件
 * 可通过 commaSeparated 参数控制是否支持逗号分隔，Tab补全，上下键选择
 */
public class SuggestionWidget {
    ...
    public SuggestionWidget(Font font, EditBox attachedBox, SuggestionProvider provider, boolean commaSeparated) {
        this.font = font;
    ...
    private void renderScrollbar(GuiGraphics guiGraphics, int dropdownX, int dropdownY,
        ...
        if (suggestions.size() <= MAX_VISIBLE) return;
