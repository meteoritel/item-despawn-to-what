...
### 新增
- **转化为世界事件**：添加了箭雨、爆炸、闪电、天气调整等世界效果，可配置字段请在GUI中查看
- **转化为经验球**：现在可以将物品转化为经验球
- **标签支持**：`itemId` 字段现在支持使用物品标签，催化剂物品现在支持使用标签
- **切比雪夫距离（圆形）放置方块**：方块放置现在支持圆形环状形状
- **TOMl 全局配置**：新增服务端 TOML 配置文件，可调整以下参数：
  - 闪电每次之间的间隔 tick 数
  - 爆炸每次之间的间隔 tick 数
  - 箭矢每支之间的间隔 tick 数
  - 方块放置任务每圈之间的间隔 tick 数
  - 方块放置圆圈形状，`SQUARE`（正方形圈）或 `CIRCLE`（圆形圈）
- **GUI 错误提示**：为非法输入框添加了红框高亮提示
- **GUI 入口调整**：移除了游戏主页面按钮入口，现在必须通过 `/conversion_config edit` 命令或快捷键（仅单人模式）打开 GUI
- **超长 JSON 分包**：添加了发包保存 JSON 时超长字符串的分包策略

### 修复
- 修复标签物品的图标无法正常显示的 bug
- 其他多项 bug 修复与性能优化